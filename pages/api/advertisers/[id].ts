...
export default async function handler(
  ...
  if (req.method === 'GET') {
    try {
      ...
      const advertiser = await AdvertiserAdminService.getById(id);
      if (advertiser) {
    ...
    try {
      ...
      // Use Admin Service to bypass Firestore security rules
      await AdvertiserAdminService.update(id, advertiserData);
      // Fetch the updated advertiser to return it
      const updatedAdvertiser = await AdvertiserAdminService.getById(id);
    ...
    try {
      // Use Admin Service to bypass Firestore security rules
      await AdvertiserAdminService.delete(id);
