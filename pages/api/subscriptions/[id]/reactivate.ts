...
/**
 * API لإعادة تشغيل اشتراك بعد إيقاف كامل
...
export default async function handler(
  ...
  try {
    ...
    const { id } = req.query;
    const { new_start_date, notes } = req.body;
    ...
    const result = await SubscriptionStatusService.reactivateSubscription({
      subscription_id: id,
      new_start_date: new_start_date ? new Date(new_start_date) : undefined,
