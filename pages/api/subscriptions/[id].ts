...
export default async function handler(
  ...
  if (req.method === 'GET') {
    try {
      ...
      const subscription = await SubscriptionAdminService.getById(id);
      if (subscription) {
    ...
    try {
      ...
      // Use Admin Service to update subscription
      await SubscriptionAdminService.update(id, subData);
      const updatedSubscription = await SubscriptionAdminService.getById(id);
    ...
    try {
      // Use Admin Service to delete subscription
      await SubscriptionAdminService.delete(id);
