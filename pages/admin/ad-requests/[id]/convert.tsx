...
export default function ConvertAdRequest() {
  ...
  const fetchRequest = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://localhost:5001/api/admin/ad-requests/${id}`, {
      ...
      setFormData(prev => ({
        ...
        services: requestData.message || '',
        plan_id: requestData.plan_id.toString(),
      }));
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create advertiser
      const advertiserResponse = await axios.post('http://localhost:5001/api/admin/advertisers', data, {
        headers: {
      ...
      // Update ad request status to approved
      await axios.put(`http://localhost:5001/api/admin/ad-requests/${id}/status`, 
        { status: 'approved' },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      toast.success('تم إضافة المعلن بنجاح');
      router.push(`/admin/advertisers/${advertiserResponse.data.id}`);
    } catch (error: any) {
  ...
  return (
    <>
      <Head>
        <title>تحويل طلب إلى معلن - {request.company_name}</title>
      </Head>
        ...
        <header className="bg-white shadow-sm sticky top-0 z-50">
          <div className="container mx-auto px-4 py-4">
            <div className="flex justify-between items-center">
              <h1 className="text-2xl font-bold text-gradient">تحويل طلب إلى معلن</h1>
              <Link href="/admin/ad-requests">
                ...
                >
                  <FaArrowLeft />
                  <span>العودة لطلبات المعلنين</span>
                </motion.button>
              </Link>
            </div>
          </div>
