...
export default function AdvertiserInvoices() {
  ...
  const fetchData = async () => {
    try {
      ...
      const advertiserRes = await axios.get(`http://localhost:5001/api/admin/advertisers/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setAdvertiser(advertiserRes.data);

      // Fetch subscriptions
      const subscriptionsRes = await axios.get(`http://localhost:5001/api/admin/advertisers/${id}/subscriptions`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setSubscriptions(subscriptionsRes.data);

      // Fetch payments
      const paymentsRes = await axios.get(`http://localhost:5001/api/admin/advertisers/${id}/payments`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setPayments(paymentsRes.data);

      // Fetch invoices
      const invoicesRes = await axios.get(`http://localhost:5001/api/admin/advertisers/${id}/invoices`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleAddPayment = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5001/api/admin/payments', {
