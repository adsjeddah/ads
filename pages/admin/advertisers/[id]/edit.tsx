...
export default function EditAdvertiser() {
  ...
  const fetchAdvertiserData = async () => {
    ...
    try {
      // Fetch plans first
      const plansResponse = await axios.get('http://localhost:5001/api/plans', { headers });
      ...
      // TODO: Replace with a direct GET /api/admin/advertisers/:id endpoint if available
      const advertiserResponse = await axios.get('http://localhost:5001/api/admin/advertisers', { headers });
      const advertiser = advertiserResponse.data.find((adv: any) => adv.id === parseInt(id as string));
      ...
      // Assuming the latest active subscription is the one to "edit"
      const subscriptionsResponse = await axios.get(`http://localhost:5001/api/admin/advertisers/${id}/subscriptions`, { headers });
      ...
      if (advertiser.icon_url) {
        ...
        // If it was an uploaded path, we might still want to show it initially, but custom uploads are now disabled for edit.
        // For simplicity, currentIconUrl can store the original icon_url if it was a path, but preview will be driven by selected_icon_name.
        if (advertiser.icon_url.startsWith('/uploads/')) {
           setCurrentIconUrl(`http://localhost:5001${advertiser.icon_url}`);
           // setPreviewIcon is removed, the preset icon preview logic will handle display based on selected_icon_name
        }
      }
  ...
  const calculateTotalAmount = () => {
    ...
    const newTotalAmount = Math.max(0, newBasePrice - discountValue);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`http://localhost:5001/api/admin/advertisers/${id}`, advertiserProfileData, {
        headers: {
      ...
      if (hasSubscriptionData && newSubscriptionStartDate && newSubscriptionEndDate) {
        ...
        await axios.post('http://localhost:5001/api/admin/subscriptions', subscriptionPayload, {
